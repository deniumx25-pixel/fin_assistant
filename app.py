...
with st.sidebar:
    ...
    # Выбор отрасли / типа предприятия
    industry = st.selectbox(
        "Отрасль / тип предприятия",
        options=[
            ...
            "Сельское хозяйство",
            "Сфера услуг",
            "Универсальные (средние по рынку)"
        ],
        index=0,
        help="Выберите отрасль для применения отраслевых нормативов коэффициентов"
    )
...
# --- Отраслевые нормативы (примерные, можно уточнить) ---
# Словарь, где ключ - название отрасли, значение - словарь с порогами
INDUSTRY_THRESHOLDS = {
    "Промышленность (производство)": {
    ...
    "Торговля / ритейл": {
        ...
        "Финансовый рычаг": (0.7, 1.5),
        "Покрытие процентов": (2, 4),
        "Оборачиваемость активов": (1.5, 3.0),
        "Оборачиваемость дебиторской задолженности": (10, 20)
    },
    "Телекоммуникации": {
        "Коэффициент текущей ликвидности": (1.2, 2.0),
        "Коэффициент быстрой ликвидности": (0.8, 1.3),
        "Коэффициент абсолютной ликвидности": (0.15, 0.4),
        "ROE": (0.12, 0.22),
        "ROA": (0.06, 0.12),
        "Рентабельность продаж": (0.15, 0.25),
        "Рентабельность до налогообложения": (0.15, 0.25),
        "Коэффициент автономии": (0.4, 0.6),
        "Финансовый рычаг": (0.6, 1.2),
        "Покрытие процентов": (3, 6),
        "Оборачиваемость активов": (0.5, 1.0),
        "Оборачиваемость дебиторской задолженности": (4, 8)
    },
    "Финансовый сектор (банки, страховые)": {
        "Коэффициент текущей ликвидности": (1.0, 1.5),  # условно
        "Коэффициент быстрой ликвидности": (0.5, 1.0),
        "Коэффициент абсолютной ликвидности": (0.1, 0.3),
        "ROE": (0.1, 0.2),
        "ROA": (0.01, 0.02),
        "Рентабельность продаж": (0.2, 0.4),
        "Рентабельность до налогообложения": (0.2, 0.4),
        "Коэффициент автономии": (0.08, 0.15),  # низкий, т.к. высокая доля заёмных средств
        "Финансовый рычаг": (5, 10),
        "Покрытие процентов": (1.5, 3),
        "Оборачиваемость активов": (0.05, 0.1),
        "Оборачиваемость дебиторской задолженности": (1, 3)
    },
    "Строительство": {
        "Коэффициент текущей ликвидности": (1.2, 2.0),
        ...
        "Покрытие процентов": (2, 4),
        "Оборачиваемость активов": (0.5, 1.0),
        "Оборачиваемость дебиторской задолженности": (3, 6)
    },
    "Транспорт и логистика": {
        "Коэффициент текущей ликвидности": (1.1, 1.8),
        "Коэффициент быстрой ликвидности": (0.7, 1.3),
        "Коэффициент абсолютной ликвидности": (0.15, 0.35),
        "ROE": (0.1, 0.2),
        "ROA": (0.04, 0.09),
        "Рентабельность продаж": (0.08, 0.15),
        "Рентабельность до налогообложения": (0.08, 0.15),
        "Коэффициент автономии": (0.4, 0.6),
        "Финансовый рычаг": (0.5, 1.0),
        "Покрытие процентов": (2.5, 4.5),
        "Оборачиваемость активов": (0.6, 1.2),
        "Оборачиваемость дебиторской задолженности": (5, 9)
    },
    "Сельское хозяйство": {
...
# Получаем пороги для выбранной отрасли
THRESHOLDS = INDUSTRY_THRESHOLDS[industry]

# Группы и веса (можно тоже сделать отраслевыми, но пока оставим общие)
GROUPS = {
